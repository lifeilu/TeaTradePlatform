...
angular.module('myApp')
    .controller('CrowdSourceOrderCtrl',function($scope, $location,$http,baseUrl) {
        ...
        var reqAdd = {
            method: 'GET',
            url: baseUrl + '/api/crowdSourcingOder/search?'
        };
        $http(reqAdd)
            .success(function (data, config, status) {
                if (data.data) {
                    $scope.rowCollection = data.data.content;
                    $scope.crowdSourceOrderNum = data.data.content.length;
